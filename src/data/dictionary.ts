...
export const answers: Answer[] = (rawAnswers as Answer[]).map((answer) => ({
  ...answer,
  hint: hints[answer.word as keyof typeof hints] ?? answer.definition,
}));
...
export function initializeDictionary(value: unknown): void {
  if (
    ...
    !value.every(
      (word) =>
        typeof word === 'string' && /^[ա-ֆև]+$/.test(word) && normalizeWord(word) === word,
    )
  )
