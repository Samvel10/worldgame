...
const seconds = Number(process.env.ROUND_SECONDS ?? 0);
...
try {
  ...
  const [a, b] = players;
  await a.page.getByLabel('Փուլի ժամանակը՝ վայրկյաններով', { exact: true }).fill(String(seconds));
  ...
  for (let round = 0; round < 4; round++) {
    ...
    expect(config.deadline).toBe(seconds === 0 ? null : config.startedAt + seconds * 1000);
    ...
    results.push({
      ...
      timesMs: state.players.map((p) => p.totalTimeMs),
    });
  ...
  for (const player of players) {
    ...
    await expect(player.page.locator('.history-list li')).toContainText('Ընդհանուր խաղաժամանակ');
  ...
  await a.page.screenshot({
    path: 'docs/screenshots/live-timing-profile-desktop.png',
    fullPage: true,
  });
  await b.page.setViewportSize({ width: 390, height: 844 });
  await b.page.screenshot({
    path: 'docs/screenshots/live-timing-profile-mobile.png',
    fullPage: true,
  });
  const reloads = [];
  for (let i = 0; i < 5; i++) {
  ...
  await writeFile(
    'docs/LIVE_TIMING_VERIFICATION.json',
    JSON.stringify(
      {
